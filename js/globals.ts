...
export function updateGlobals(params: any) {
    ...
    for (let i of params.rotation)
        for (let j of spells)
            if (i.id == j.id)
                for (let prop in i)
                    if (prop != 'minlevel' && prop != 'maxlevel' && prop != 'iconname') j[prop] = i[prop];

    for (let type in gear) for (let j of gear[type]) if (j.selected) j.selected = false;
    ...
    for (let type in enchant) for (let j of enchant[type]) if (j.selected) j.selected = false;
    ...
    if (typeof runes !== 'undefined') {
        for (let type in runes) for (let j of runes[type]) if (j.selected) j.selected = false;
    }
