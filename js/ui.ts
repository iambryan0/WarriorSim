...
SIM.UI = {
    ...
    events: function () {
        ...
        view.main.find('nav li p, nav li img').click(function (e) {
            ...
            if (type == 'mainhand' || type == 'offhand' || type == 'twohand') view.loadWeapons(type);
        ...
        view.runes.on('click', '.rune .icon', function (e) {
            ...
            else if (current_open_page == 'custom') view.loadCustom();
            else view.loadGear(current_open_page);
        });
        ...
        view.tcontainer.on('keyup', 'input[name="search"]', function (e) {
            ...
            view.tcontainer.find('.gear td:nth-child(2)').each(function () {
                let td = $(this).get(0);
                if (!val || td.textContent.toLowerCase().indexOf(val.toLowerCase()) > -1)
                    td.parentElement.classList.remove('filtered');
    ...
    enableEditMode: function () {
        ...
        if (type == 'mainhand' || type == 'offhand' || type == 'twohand') view.loadWeapons(type, true);
    ...
    disableEditMode: function () {
        ...
        let type = view.tcontainer.find('table.gear').attr('data-type');
        if (type == 'mainhand' || type == 'offhand' || type == 'twohand') view.loadWeapons(type, false);
    ...
    simulateDPS: function (rows) {
        ...
        var sim = new SimulationWorkerParallel(
            ...
            (report) => {
                ...
                stats.html(
                    report.mindps.toFixed(2) + ' min&nbsp;&nbsp;&nbsp;&nbsp;' + report.maxdps.toFixed(2) + ' max',
                );
                btn.css('background', '');
    ...
    simulateWeights: function (player, mean, varmean) {
        ...
        const simulateWeight = (stat, amount) =>
            this.simulateStat(stat, amount, updateFn).then((result) => {
                tasksDone += 1;
                return {
                    weight: (result.mean - mean) / amount,
                    error: (1.96 * Math.sqrt(varmean + result.varmean)) / amount,
                };
            });
            ...
            simulation.run = () => {
                ...
                this.simulateRow($(row), (perc) => {
                    ...
                    if (total == 100) {
                        ...
                        btn.css(
                            'background',
                            'linear-gradient(to right, transparent ' + total + '%, #444 ' + total + '%)',
                        );
                    }
    ...
    simulateRow: function (tr, updateFn) {
        ...
        var sim = new SimulationWorker(
            (report) => {
                ...
                if (isench) {
                    for (let i of enchant[type]) if (i.id == item) i.dps = calc.toFixed(2);
                } else {
    ...
    rowEnableItem: function (tr) {
        ...
        for (let i = 0; i < gear[type].length; i++) {
            ...
            else if (type != 'finger' && type != 'trinket' && type != 'custom') gear[type][i].selected = false;
        }

        if (type == 'twohand') {
            for (let i = 0; i < gear.mainhand.length; i++) gear.mainhand[i].selected = false;
            for (let i = 0; i < gear.offhand.length; i++) gear.offhand[i].selected = false;
            for (let i = 0; i < enchant.mainhand.length; i++) enchant.mainhand[i].selected = false;
            for (let i = 0; i < enchant.offhand.length; i++) enchant.offhand[i].selected = false;
        }

        if (type == 'mainhand' || type == 'offhand') {
            for (let i = 0; i < gear.twohand.length; i++) gear.twohand[i].selected = false;
            for (let i = 0; i < enchant.twohand.length; i++) enchant.twohand[i].selected = false;
        }

        for (let i = 0; i < spells.length; i++) {
            if (
                spells[i].item &&
                spells[i].id == tr.data('id') &&
                !spells[i].timetoendactive &&
                !spells[i].timetostartactive
            ) {
                // Blademasters Fury
    ...
    rowDisableEnchant: function (tr) {
        ...
        for (let i = 0; i < enchant[type].length; i++) {
            if (enchant[type][i].id == tr.data('id')) enchant[type][i].selected = false;
        }
    },
    ...
    rowEnableEnchant: function (tr) {
        ...
        tr.addClass('active');
        for (let i = 0; i < enchant[type].length; i++) {
            if (enchant[type][i].id == tr.data('id')) enchant[type][i].selected = true;
        }
    },
        ...
        for (let i = 0; i < runes[type].length; i++) {
            if (runes[type][i].id == div.data('id')) {
                ...
                if (runes[type][i].enable) {
                    for (let spell of spells) if (spell.id == runes[type][i].enable) spell.active = false;
                    for (let buff of buffs) {
                        if (buff.id == runes[type][i].enable) buff.active = false;
                        if (buff.id == 2458 && runes[type][i].gladstance) buff.active = true;
                    }
                }
            }
        }
    ...
    rowEnableRunes: function (div) {
        ...
        for (let i = 0; i < runes[type].length; i++) {
            if (runes[type][i].id == div.data('id')) {
                ...
                if (runes[type][i].enable) {
                    for (let spell of spells) if (spell.id == runes[type][i].enable) spell.active = true;
                    for (let buff of buffs) {
                        if (buff.id == runes[type][i].enable) buff.active = true;
                        if (
                            runes[type][i].buffgroup &&
                            buff.group == runes[type][i].buffgroup &&
                            buff.id !== runes[type][i].enable
                        )
                            buff.active = false;
                    }
                }
    ...
    updateSidebar: function () {
        ...
        view.sidebar
            ...
            .html(
                player.stats['skill_' + player.mh.type] +
                    ' <small>MH</small>' +
                    (player.oh ? space + player.stats['skill_' + player.oh.type] + ' <small>OH</small>' : ''),
            );
        ...
        view.sidebar
            ...
            .html(
                Math.max(player.mh.miss, 0).toFixed(2) +
                    '% <small>1H</small>' +
                    (player.oh ? space + Math.max(player.mh.dwmiss, 0).toFixed(2) + '% <small>DW</small>' : ''),
            );
        view.sidebar
            .find('#dodge')
            .html(
                player.mh.dodge.toFixed(2) +
                    '% <small>MH</small>' +
                    (player.oh ? space + player.oh.dodge.toFixed(2) + '% <small>OH</small>' : ''),
            );
        ...
        for (let set of sets) {
            ...
            if (counter >= set.bonus[0].count)
                view.sidebar
                    .find('#sets')
                    .append(`<a href="${WEB_DB_URL}item-set=${set.id}" class="q4">${set.name} (${counter})</a><br />`);
        }
    ...
    updatePaperdoll: function () {
        ...
        for (let type in gear) {
            ...
            let ench;
            if (enchant[type]) {
                for (let e of enchant[type]) if (e.selected && e.ench) ench = e.ench;
            }
            ...
            $(`nav.paperdoll [data-type="${type}"] img`).attr(
                'src',
                `https://wow.zamimg.com/images/wow/icons/medium/${path}.jpg`,
            );
        }
    },
    ...
    loadSession: function () {
        ...
        if (storage.targetbasearmor === 3731 || storage.targetbasearmor === null) storage.targetbasearmor = 3128;
        ...
        for (let i of _sources) view.filter.find(`.sources [data-id="${i}"]`).addClass('active');

        for (let i of _phases) view.filter.find(`.phases [data-id="${i}"]`).addClass('active');
    ...
    loadWeapons: function (type, editmode) {
        ...
        for (let item of gear[type]) {
            if (
                !item.selected &&
                (item.r > level ||
                    (mode == 'sod' && item.q < 3 && item.i < level - 7) ||
                    (mode == 'sod' && item.q == 3 && item.i < level - 10) ||
                    (mode == 'sod' && item.q == 4 && item.i < level - 15))
            ) {
                continue;
            }
            ...
            if (filter) {
                if (filter == 'All') {
                    if (type == 'offhand' && storage.buffs.includes('71')) {
                        // Glad & Def Stance
            ...
            if (
                item.subsource == 'shadow' ||
                item.subsource == 'arcane' ||
                item.subsource == 'nature' ||
                item.subsource == 'fire' ||
                item.subsource == 'frost'
            )
                source = 'resistances-list';

            if (phase && !view.filter.find('.phases [data-id="' + phase + '"]').hasClass('active')) continue;
            if (source && !view.filter.find('.sources [data-id="' + source + '"]').hasClass('active')) continue;

            if (
                source === 'resistances-list' &&
                !$(".resistances[data-id='" + item.subsource + "-resist']").prop('checked')
            ) {
                continue;
            }
            ...
            let tooltip = item.id.toString().split('|')[0],
                rand = '';
    ...
    loadEnchants: function (type, editmode) {
        ...
        for (let item of enchant[type]) {
            ...
            if (item.phase && !view.filter.find('.phases [data-id="' + item.phase + '"]').hasClass('active')) continue;
    ...
    addAlert: function (msg) {
        ...
        setTimeout(function () {
            view.alerts.find('#alert' + rng).addClass('in-up');
        });
        setTimeout(function () {
            view.closeAlert(rng);
        }, 4000);
    ...
    closeAlert: function (rng) {
        ...
        setTimeout(function () {
            view.alerts.find('#alert' + rng).remove();
        }, 1000);
    },
